          ...
          <form @submit.prevent="updateUser">
            ...
            <div class="form-group">
              ...
              <input
                type="text"
                v-model="user.city"
                placeholder="Masukkan kota user"
                class="form-control"
              />
            ...
            <div class="form-group">
              ...
              <input
                ...
                placeholder="Masukkan nomor rumah user"
                class="form-control"
              />
              <div v-if="validation.houseNumber" class="mt-2">
                <b-alert show variant="danger">{{
                  validation.houseNumber[0]
                }}</b-alert>
              </div>
            </div>
            <div class="form-group">
              ...
              <input
                ...
                placeholder="Masukkan nomor telp user"
                class="form-control"
              />
              <div v-if="validation.phoneNumber" class="mt-2">
                <b-alert show variant="danger">{{
                  validation.phoneNumber[0]
                }}</b-alert>
              </div>
            </div>
            <div class="form-group">
...
export default {
  ...
  head() {
    return {
      title: "Edit user - Coffe Shop - Android Corners",
    };
  },
  ...
  mounted() {
    //fetching data user
    this.$axios
      .get(`/api/dashboard/users/${this.$route.params.id}`)

      .then((response) => {
        this.user.name = response.data.data.name;
        this.value = response.data.data.roles;
        ...
        this.user.houseNumber = response.data.data.houseNumber;
      });
  },
  ...
  methods: {
    ...
    async updateUser() {
      ...
      await this.$axios
        .post(`/api/dashboard/users/${this.$route.params.id}`, formData)
        .then(() => {
          //sweet alert
          this.$swal.fire({
            title: "BERHASIL!",
